...
The file is generated by the `methurator gt-estimator` command using the Good-Toulmin (GT) estimator for sequencing saturation extrapolation.
...
Contains Good-Toulmin extrapolation results, grouped by sample and minimum coverage threshold.
...
| `extrapolated` | string    | "True" if t > 1.0 (extrapolated), "False" if t ≤ 1.0 (interpolated) |
