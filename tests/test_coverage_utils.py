...
from methsaturator.coverage_utils import mincoverage_checker
...
def test_zero_values_are_ignored(monkeypatch):
    ...
    monkeypatch.setattr("methsaturator.coverage_utils.vprint", fake_vprint)
...
def test_only_zero_values_returns_empty_list(monkeypatch):
    monkeypatch.setattr(
        "methsaturator.coverage_utils.vprint", lambda *args, **kwargs: None
    )
